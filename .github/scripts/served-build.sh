...
# =============================================================================
# Reads commitSha from the authorized /api/health response. The rollback
# workflow asks this three times - the slot and production before the swap, and
# production after it - and the answer decides whether a swap happens at all.
#
...
# and it is never retried, because no amount of waiting adds a field the
# response cannot carry. Reporting it as a build mismatch would send the reader
# to the image when the problem is an app setting.
#
