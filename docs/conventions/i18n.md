...
`product(alias:, languageId:)` returns `null` when the product is not published in the requested language channel — translating the name and texts in the admin is not sufficient on its own, the product entity must also be activated for that channel.

| Scenario                                               | Behavior                                                                                                  |
| ------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| Translation exists for requested locale                | Returns translated fields                                                                                 |
| Requested locale returns `null`, default locale has it | `getProduct()` retries once with the SDK default `languageId` — PDP renders with default-language content |
| Both locales return `null`                             | Throws `NOT_FOUND`; route sets `Cache-Control: no-store` so the CDN does not pin the miss                 |

The retry lives in `server/services/products.ts` and is the single place to extend if Geins later exposes per-language alias resolution. Do not duplicate the fallback in route handlers or components — they should treat a non-null product as authoritative regardless of which locale it came from.

See [ADR-005 — Product Language Fallback](../adr/005-internationalization.md#product-language-fallback) for the full rationale.
