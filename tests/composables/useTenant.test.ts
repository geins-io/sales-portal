...
describe('useTenant', () => {
  ...
  describe('hasFeature', () => {
    ...
    describe('override.features resolution', () => {
      it('returns false when base is enabled but override disables it', () => {
        mockData.value = createMockTenantConfig({
        ...
        expect(hasFeature('b2bQuotes')).toBe(false);
      });

      it('returns true when base is disabled but override enables it', () => {
        mockData.value = createMockTenantConfig({
          features: { b2bQuotes: { enabled: false } },
          overrides: {
            features: { b2bQuotes: { enabled: true } },
        ...
        expect(hasFeature('b2bQuotes')).toBe(true);
      });

      it('returns true when base is missing but override enables it', () => {
        mockData.value = createMockTenantConfig({
          features: {},
          overrides: {
        ...
        expect(hasFeature('newThing')).toBe(true);
      });

      it('falls back to base when override is absent', () => {
        mockData.value = createMockTenantConfig({
          features: { search: { enabled: true } },
        });
      ...
      it('replaces base entry rather than deep merging', () => {
        // Base { enabled: true, access: 'all' }, override { enabled: true, access: 'authenticated' }
        // Override entry replaces base entry. The resulting feature reflects
        // override.access, not base.access.
        mockData.value = createMockTenantConfig({
          features: { promo: { enabled: true, access: 'all' } },
          overrides: {
            features: {
              promo: { enabled: true, access: 'authenticated' },
            },
          },
        } as Partial<PublicTenantConfig>);

        const { features } = useTenant();
        const resolved = features.value?.promo;

        expect(resolved?.enabled).toBe(true);
        expect(resolved?.access).toBe('authenticated');
      });
    });
  });
