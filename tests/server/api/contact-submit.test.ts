...
vi.stubGlobal('contactFormRateLimiter', { check: mockRateLimiterCheck });
vi.stubGlobal(
  'createAppError',
...
describe('POST /api/contact/submit', () => {
  ...
  beforeEach(async () => {
    ...
    if (!handler) {
      const mod = await import('../../../server/api/contact/submit.post');
  ...
  it('throws validation error for invalid body', async () => {
    capturedBody = { name: '', email: 'bad', subject: '', message: '' };
