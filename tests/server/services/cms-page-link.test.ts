...
describe('getPageLinkByTag', () => {
  ...
  beforeEach(async () => {
    ...
    getRequestChannelVariablesMock = sdkMod.getRequestChannelVariables as ReturnType<typeof vi.fn>;
  ...
  it('e. locale isolation: sv-SE and en-US calls use distinct cache keys', async () => {
    getRequestChannelVariablesMock
      .mockReturnValueOnce({ channelId: 'ch1', languageId: 'sv-SE', marketId: 'se' })
      .mockReturnValueOnce({ channelId: 'ch1', languageId: 'en-US', marketId: 'se' });
    ...
    mockQuery
      .mockResolvedValueOnce({
        cmsPages: [{ alias: 'kontakt', tags: ['contact'], canonicalUrl: '/se/sv/kontakt' }],
      })
      .mockResolvedValueOnce({
        cmsPages: [{ alias: 'contact', tags: ['contact'], canonicalUrl: '/se/en/contact' }],
      });
